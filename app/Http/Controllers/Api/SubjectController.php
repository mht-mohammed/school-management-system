...
class SubjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Pre-fetch sections for all grade levels (avoid N+1)
        $sectionsByGrade = \App\Models\Section::whereIn('grade_level_id', $gradeIds)
            ->get()
            ->groupBy('grade_level_id')
            ->map(fn($secs) => $secs->pluck('id')->toArray())
            ->toArray();
        ...
        $result = $subjects->map(function ($subject) use ($teachers, $sectionsByGrade) {
            $sectionsInGrade = $sectionsByGrade[$subject->grade_level_id] ?? [];
