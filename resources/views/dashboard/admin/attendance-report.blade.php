    ...
    <script>
        ...
        function importAttendance() {
            ...
            fetch(API_BASE + '/admin/attendance/import', {
                ...
                let html = `<div style="background:#d4edda;color:#155724;padding:15px;border-radius:8px;margin-bottom:15px;font-weight:700;">✅ ${result.message}</div>`;

                if (result.not_found?.length) {
                    html += `<div style="background:#fff3cd;color:#856404;padding:15px;border-radius:8px;margin-bottom:15px;font-size:13px;">
                ...
                if (result.summary?.length) {
                    ...
                    result.summary.forEach(s => {
                        ...
                        html += `<tr><td>${avatar}${s.student_name}</td><td>${toArabicNum(s.total)}</td><td style="color:#28a745;">${toArabicNum(s.present)}</td><td style="color:#dc3545;">${toArabicNum(s.absent)}</td><td><strong>${toArabicNum(s.percentage)}%</strong></td></tr>`;
                    });
                    html += '</tbody></table>';
                }
            ...
            apiFetch(url).then(list => {
                ...
                Object.entries(byClass).forEach(([cid, cls]) => {
                    ...
                    Object.values(cls.students).forEach(s => {
                        ...
                        const avatar = s.avatar ? `<img src="/storage/${s.avatar}" style="width:24px;height:24px;border-radius:50%;object-fit:cover;vertical-align:middle;margin-left:6px;">` : '';
                        html += `<tr><td>${avatar}${s.name}</td><td>${toArabicNum(s.total)}</td><td style="color:#28a745;">${toArabicNum(s.present||0)}</td><td style="color:#dc3545;">${toArabicNum(s.absent||0)}</td><td><strong>${toArabicNum(pct)}%</strong></td></tr>`;
                    });
                    html += '</tbody></table>';
                });
