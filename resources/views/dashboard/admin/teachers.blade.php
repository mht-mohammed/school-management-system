...
@section('content')
    ...
    <div id="teacherFormModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:999;align-items:center;justify-content:center;">
        <div style="background:#fff;padding:28px;border-radius:var(--radius-md);width:90%;max-width:500px;max-height:90vh;overflow-y:auto;">
            ...
            <div class="form-group"><label>{{ __('تاريخ التعيين') }}</label><input id="fHireDate" type="date" style="width:100%;padding:10px;border:1px solid #ddd;border-radius:8px;margin:4px 0 16px;"></div>
                ...
                <div style="display:flex;align-items:center;gap:12px;margin:4px 0 12px;">
                    ...
                    <span id="fRemoveAvatarBtn" style="display:none;cursor:pointer;color:#b8232e;font-weight:600;font-size:13px;" onclick="removeTeacherAvatar()">{{ __('🗑️ حذف') }}</span>
                </div>
            </div>
    ...
    <script>
        ...
        function renderTeachers(list) {
            ...
            if (groupedView) {
                ...
                sortedSpecs.forEach(spec => {
                    const translatedSpec = subjectName(spec) || spec;
                    tbody.innerHTML += `<tr style="background:#f0f4f8;font-weight:700;"><td colspan="9" style="padding:10px 14px;font-size:15px;">📂 ${translatedSpec} (${groups[spec].length})</td></tr>`;
        ...
        function renderRow(u, tbody) {
            ...
            const wlColor = wl === 0 ? 'color:#b8232e;font-weight:700;' : '';
            ...
            tbody.innerHTML += `<tr>
                ...
                <td>${t.salary ? t.salary + ' ' + __('دولار') : '-'}</td>
    ...
    <style>
        ...
        .badge-class.primary { background:#e8f0fe;color:var(--blue-main); }
