...
@section('content')
    ...
    <div id="subjectModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:999;align-items:center;justify-content:center;">
        <div style="background:#fff;padding:25px;border-radius:14px;width:90%;max-width:400px;">
    ...
    <script>
        ...
        function renderSubjects() {
            ...
            allSubjects.forEach((s, i) => {
                const teacherHtml = (s.teachers || []).length
                    ? s.teachers.map(t => `<span style="display:inline-block;padding:2px 12px;border-radius:20px;background:#e8f0fe;color:#1a73e8;font-weight:600;font-size:12px;margin:2px;">${t.name}</span>`).join('')
                    : '<span style="color:#dc3545;">' + __('غير معيّن') + '</span>';
                tbody.innerHTML += `<tr>
        ...
        function deleteSubject(id, name) {
            if (!confirm(__('هل أنت متأكد من حذف المادة') + ' "' + name + '" ' + __('؟ سيتم حذف جميع الدرجات المسجلة لهذه المادة.'))) return;
            apiFetch('/admin/subjects/' + id, { method: 'DELETE' }).then(r => {
                showToast(r.message || __('✅ تم حذف المادة'));
                filterByClass();
            });
        }
    </script>
