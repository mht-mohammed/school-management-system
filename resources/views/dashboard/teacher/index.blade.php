    ...
    <script>
        ...
        apiFetch('/teacher/classes').then(async data => {
            ...
            for (const c of data) {
                ...
                // Fetch grades for this section
                let grades = [];
                try {
                    grades = await apiFetch('/teacher/grades?section_id=' + c.id);
                } catch(e) {}

                // Build grade map: student_id -> final grade score
                const gradeMap = {};
                (grades || []).forEach(g => {
                ...
                const studentRows = (c.students || []).map((s, i) => {
                    ...
                    const gradeColor = total !== undefined ? (total >= 50 ? '#2e7d32' : '#c62828') : '#ccc';
                    return `<tr>
                ...
                div.innerHTML += `
                    <div style="border:1px solid #e0e0e0;border-radius:14px;padding:16px;margin-bottom:12px;">
