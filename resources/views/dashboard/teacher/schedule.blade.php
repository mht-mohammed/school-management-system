...
@section('content')
        ...
        apiFetch('/teacher/schedule').then(data => {
            ...
            daysOrder.forEach(day => {
                ...
                for (let p = 1; p <= 5; p++) {
                    const s = grid[day][p];
                    if (s) {
                        html += `<td style="background:var(--blue-50);color:var(--blue-main);font-size:13px;">
    ...
    <style>
        #scheduleGrid td { padding: 10px 6px; text-align: center; vertical-align: middle; min-width: 100px; border:1px solid var(--border-soft); }
        #scheduleGrid th { text-align: center; font-size: 13px; background:linear-gradient(135deg,var(--blue-main),var(--blue-light)); color:#fff; padding:10px; border:1px solid var(--blue-main); }
        #scheduleGrid tr td:first-child { background:var(--blue-50); font-weight:700; }
    </style>
@stop
