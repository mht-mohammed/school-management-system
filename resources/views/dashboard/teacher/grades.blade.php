    ...
    <div class="card">
        ...
        <div id="tabAddContent">
            ...
            <div style="background:#f9f9f9;border-radius:10px;padding:15px;margin-bottom:20px;">
                ...
                <div id="distMsg" style="margin-top:8px;padding:8px 12px;border-radius:8px;background:#fff3cd;color:#856404;font-size:13px;">
            ...
            <div id="prefilledInfo" style="display:none;background:#fff3cd;padding:10px;border-radius:8px;margin-bottom:15px;font-size:13px;"></div>
    ...
    <script>
        ...
        function saveDistribution() {
            ...
            apiFetch('/teacher/grades/distribution', {
                ...
                updateDownloadBtn();
                document.getElementById('distResult').innerHTML = '<div style="padding:10px;background:#d4edda;color:#155724;border-radius:8px;">' + r.message + '</div>';
            }).catch(e => {
                ...
                fetch(url, {
                    ...
                    if (d.confirmation_required) {
                        ...
                        if (!confirm(confirmMsg)) {
                            resultDiv.innerHTML = '<div style="margin-top:10px;padding:12px;border-radius:8px;background:#fff3cd;color:#856404;">❌ ' + __('تم إلغاء الاستيراد.') + ' ' + __('التوزيعة القديمة:') + ' ' + oldList + '</div>';
                            return;
                        }
                        ...
                        .then(d2 => {
                            let html = '<div style="margin-top:10px;padding:12px;border-radius:8px;background:#d4edda;color:#155724;">';
                            ...
                            if (d2.errors && d2.errors.length) {
                                ...
                                if (realErrors.length) {
                                    html += '<div style="margin-top:8px;padding:10px;border-radius:8px;background:#f8d7da;color:#721c24;font-size:13px;"><strong>' + __('أخطاء:') + '</strong><ul>';
                                    realErrors.forEach(e => { html += '<li>' + e + '</li>'; });
                                    html += '</ul></div>';
                                }
                                if (warnings.length) {
                                    html += '<div style="margin-top:8px;padding:10px;border-radius:8px;background:#fff3cd;color:#856404;font-size:13px;"><strong>' + __('ملاحظات:') + '</strong><ul>';
                            ...
                            resultDiv.innerHTML = '<div style="margin-top:10px;padding:12px;border-radius:8px;background:#f8d7da;color:#721c24;">' + __('خطأ:') + ' ' + err.message + '</div>';
                        });
                        return;
                    }
                    let html = '<div style="margin-top:10px;padding:12px;border-radius:8px;background:#d4edda;color:#155724;">';
